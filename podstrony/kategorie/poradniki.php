<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="google-adsense-account" content="ca-pub-1518890929804230">
    <title>CapForum</title>
    <link rel="stylesheet" href="/mybulma/css/customstyle.css/customstyle.css?v=5" type="text/css">
    <style>
html, body {
  overflow-x: hidden;  
}
</style> 
  </head>
  <body>
    
  <?php include_once $_SERVER['DOCUMENT_ROOT'] . "/szablon/nav-bar.php"; ?>

    <div class="container" style="align-items: center;">
      <div class="is section is-large">
        <div class="box has-background-black-ter is-rounded is-outlined">
        <h1 class="is title has-text-white has-text-centered has-text-weight-bold">Poradniki</h1><br><h1 class="is title has-text-centered has-text-weight-bold is-size-4" style="color: red;" >Artykuły:</h1>
        <div class="has-text-white is-size-4"><a href="/podstrony/kategorie/katpor/Terraria">Terraria</a>

        </div>
          <p class="is is-size-1"><br></p>
        </div>
      </div>

    </div>



    <p class="is-size-1"><br><br><br><br><br>

  </body>

  <?php include_once $_SERVER['DOCUMENT_ROOT'] . "/szablon/stopka.php"; ?>
</html>
